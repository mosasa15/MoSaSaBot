...
let pro = {
    ...
    computeManorWithCache(roomName, points, blocked) {
        ...
        // 1. 检查缓存
        if (global.layoutCache[roomName]) {
            console.log(`[A56] 使用缓存布局: ${roomName}`);
            return global.layoutCache[roomName];
        }

        // 2. 正常计算
        const result = this.computeManor(roomName, points, blocked);
        if (result) {
            global.layoutCache[roomName] = result;
            console.log(`[A56] 已缓存布局: ${roomName}`);
        }
    ...
    computeBlock(roomName, blocked) {
        ...
        // ========================================================
        // 改进版：每个 source 旁放 container + 2 link
        // ========================================================
        for (let i = 0; i < objects.length; i++) {
            ...
            if (i === 1) {
                ...
                roomObjectCache.container.push(containerPos)
                continue
            }
            ...
            if (i === 0) {
                ...
                roomObjectCache.container = roomObjectCache.container || []
                roomObjectCache.container.unshift(containerPos)
                continue
            }
            ...
            const nearSpots = []
            roomPutAble.forNear((x, y, val) => {
                if (val && !roomObjectCache.get(x, y)) nearSpots.push([x, y])
            ...
            roomObjectCache.link = roomObjectCache.link || []
            if (link1) roomObjectCache.link.push(link1)
            if (link2) roomObjectCache.link.push(link2)
    ...
    computeManor(roomName, points, blocked) {
        ...
        ["spawn", "nuker", "powerSpawn", "tower", "observer"].forEach(struct => {
            ...
            let alreadyPlaced = structMap[struct].length; // 这里记录你中心布局已经放了几个
        ...
        for (let i = 0; i < 50; i++) {
            for (let j = 0; j < 50; j++) {
                ...
                costs.set(i, j,
                    te == TERRAIN_MASK_WALL ? 255 :
                        (te == TERRAIN_MASK_SWAMP ? 4 : 2)
                );
            }
        }

        // 把现有建筑当成障碍，把现有 road 设为低权重
        ...
        if (structMap["storage"] && structMap["storage"][0]) {
            ...
            structMap["container"].forEach(e => {
                ...
                // ⭐ 关键：真正把 path 里的格子写进 road，并更新 costMatrix
